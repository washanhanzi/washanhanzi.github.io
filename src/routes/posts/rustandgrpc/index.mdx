...
So I told myself maybe I can remove all the `static dispatching` and `async_trait` stuff. Let's name this method `no trait`. I can tell after removing `trait` and `generic`, the code becomes much cleaner and the final docker image size is only about `60Mb`, almost halving the build size.
...
I also tried `dynamic dispatching`, build size wise, it's almost identical to `no trait` method.
...
## where i should put the clone trait? idk rely on document

## tonic-build tip --matching proto folder structure

## match hell on param validating
